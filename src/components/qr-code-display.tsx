...
export default function QrCodeDisplay({ alias }: QrCodeDisplayProps) {
  ...
  const handleShare = async () => {
    const shareData = {
        title: 'Mon QR Code PAYTIK',
        text: `Payez-moi facilement avec PAYTIK! Mon alias est ${alias}`,
    ...
    try {
        if (navigator.share) {
            ...
            toast({
                ...
                description: "Votre alias PAYTIK a été copié.",
            });
        }
    } catch (err) {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-4 text-primary">Votre QR Code PAYTIK</h2>
      <p className="text-muted-foreground mb-6">Partagez votre QR code pour recevoir des paiements facilement.</p>
      <Card className="max-w-sm mx-auto text-center shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Votre Alias: {alias}</CardDescription>
        </CardHeader>
